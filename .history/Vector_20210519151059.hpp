...
namespace ft
{
	...
	public:
		...
		explicit vector(const allocator_type &alloc = allocator_type()) : _capacity(NULL), _v_begin(NULL), _v_end(NULL), _allocator_type(alloc) {}

		// #2: fill constructor: constructs a container with n elements. Each element is a copy of val.
		explicit vector(size_type n, const value_type &val = value_type(),
						const allocator_type &alloc = allocator_type()) : _capacity(NULL), _v_begin(NULL), _v_end(NULL), _allocator_type(alloc)
		{
			if (n)
				assign(n, val);
		}
		...
		{
			return static_cast<size_type>(this->_capacity - this->_v_begin + 1);
		}
