...
/*   Updated: 2021/04/23 19:51:23 by dbliss           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
    ...
    struct myIterator_tag : bidirectional_iterator_tag
    {
    };
...
public:
                    ...
                    ConstVectorIterator(ConstVectorIterator const &copy) {*this = copy;}
    ...
    template <class T>
    class myConstIterator : public myIterator<T> // нужно конечно же будет изменить для каждого типа!!! когда сделаю assign
    {
    public:
        typedef typename ft::iterator_traits<T>::iterator_category iterator_category;
        typedef typename ft::iterator_traits<T>::value_type value_type;
        ...
        explicit myConstIterator(const T &ptr) : myIterator<T>(ptr) {} // initialization constructor

        template <typename Y>
        myConstIterator(const myConstIterator<Y> &rhs) : *this = rhs {} // copy constructor
        ...
        myConstIterator &operator=(myConstIterator const &rhs) // asignment operator
        {
            this->_ptr = rhs._ptr;
            return (*this);
        }

        ~myConstIterator() {} // destructor
